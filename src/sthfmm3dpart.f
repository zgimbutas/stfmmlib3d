...
c
        allocate( pot0(3, nsource) )
        allocate( pre0(nsource) )
        allocate( grad0(3,3, nsource) )
c
...
c
c
        subroutine sth3dpartdirecttime(itype,ms,
     $     nsource,source,
     $     ifsingle,sigma_sl,ifdouble,sigma_dl,sigma_dv,
     $     ifpot,pot,pre,ifgrad,grad,ntarget,
     $     target,ifpottarg,pottarg,pretarg,
...
c
        if( itype .eq. 1 ) then
c
        call st3dpartdirecttime(ms,
     $     nsource,source,
     $     ifsingle,sigma_sl,ifdouble,sigma_dl,sigma_dv,
     $     ifpot,pot,pre,ifgrad,grad,ntarget,ntarget,
     $     target,ifpottarg,pottarg,pretarg,
     $     ifgradtarg,gradtarg)
c
        endif
...
c
        ...
        call st3dpartdirecttime(nsource,
     $     nsource,source_image,
     $     ifsingle,sigma_sl_image,
     $     ifdouble,sigma_dl_image,sigma_dv_image,
     $     ifpot0,pot0,pre0,ifgrad0,grad0,ms,ntarget0,
     $     target0,ifpottarg0,pottarg0,pretarg0,
     $     ifgradtarg0,gradtarg0)
...
c
        ...
        call l3dpartquaddirecttime(nsource,nsource,
     $     source_image,ifcharge,charge,ifdipole,dipstr,dipvec,
     $     ifquad,quadstr,quadvec,
     $     ifpot0,cpot0,iffld0,cfld0,ifhess0,chess0,ms,ntarget0,
     $     target0,ifpottarg0,cpottarg0,iffldtarg0,cfldtarg0,
     $     ifhesstarg0,chesstarg0)
c
c
